...
if [ $? -ne 0 ]; then
  echo "Unable to find any JDKs. Using the current JRE."
...
JAVA_PARAMETERS="-showversion -classpath lib/\* -XX:+UseG1GC -Xdock:name='MZminePI' -Xdock:icon=icons/MZmineIcon.png -Djava.io.tmpdir=$TMP_FILE_DIRECTORY -Dapple.laf.useScreenMenuBar=true -Xms1024m -Xmx${HEAP_SIZE}m"
